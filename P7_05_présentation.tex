...
\begin{frame}{\insertsection}{\insertsubsection}
    \begin{itemize}
        \item[] Pour chaque solution de rééquilibrage retenues :
        \begin{itemize}
            \item[] RandomOverSampler, RandomUnderSampler, SMOTE, SMOTETomek
        ...
        \item[]
        \item[] Nous avons fait différents essais d'optimisation avec plusieurs métriques
    \end{itemize}
\end{frame}
...
\begin{frame}{\insertsection}
    \begin{itemize}
        \item Utilisation de données déséqulibrées
        \item[]
        \item Optimisation des modèle à l'aide d'une GridSearch
        \item[]
        \item Utilisation d'une métrique plus particulière afin d'optimiser le modèle en fonction des attentes du métier
        \item[]
        \item Il aurait pû être intéressant d'optimiser plus d'hyperparamètres afin d'améliorer un peu plus nos résultats
        \item[]
